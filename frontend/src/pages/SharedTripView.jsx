...
import { 
  ...
  FaBriefcase, FaComments, FaBook, FaClock, FaStar,
  FaSun, FaCloud, FaCloudShowersHeavy, FaBolt, FaSnowflake, FaWind, FaTint
} from 'react-icons/fa';
        ...
        <div className="p-4 p-md-5">
          {activeTab === 'itinerary' && (
            <div className="animate-fade-in-up">
              ...
              <div className="d-flex flex-wrap gap-2 mb-4">
                {Array.from(new Set(trip.itinerary?.map(d => d.day))).map((dayNum) => (
                  <button
                    ...
                    className={`btn btn-sm px-3.5 py-2 rounded-pill fw-semibold ${Number(selectedDay) === Number(dayNum) ? 'btn-success' : 'btn-outline-secondary'}`}
                  >
                    Day {dayNum}
                  </button>
                ))}
              ...
              {/* Schedule Timeline */}
              {activeDayPlan && (
                <div className="position-relative ms-2 ps-4 border-start border-2 border-success-subtle pb-3">
